...
```{r ans11}
index1 <- which(transcriptData$學號=="edfeQ7fb8")
subsample <- transcriptData[index1,]
subsample
```
...
index2 <- which(transcriptData$學號==x)
subsample <- transcriptData[index2,]
subsample$學期成績 >= 60 -> subsample$及格
   總修習學分數=sum(subsample$學分數)
   總實得學分數=sum(subsample$學分數[subsample$及格])
   data.frame(學號=x, 總修習學分數=總修習學分數, 總實得學分數=總實得學分數)
}
...
```{r ans16}
wowzers <- str_which(transcriptData$`必選修類別（必∕選∕通）`,"通")
通識科目總表 <- unique(transcriptData[wowzers,"科目名稱"])
通識科目總表
```
...
```{r ans18}
lastanswer <- str_which(transcriptData$科目名稱, "ZT13")
yay <- transcriptData[lastanswer,"系別"]
shannon(yay)
```
